...
TEST_TEMP_DIR = tempfile.mkdtemp(prefix="lib-cove-oc4ids-tests", dir=tempfile.gettempdir())


def get_validation_errors(tempdir):
    """returns the validation errors for the last run check"""
    validation_json_path = os.path.join(tempdir, "validation_errors-3.json")
...
def test_fixture(fixture_name):
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        context = oc4ids_json_output(tmpdirname, get_path_for_fixture(fixture_name), "json")
